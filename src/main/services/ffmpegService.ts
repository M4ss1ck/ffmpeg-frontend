...
class FFmpegService {
    ...
    async detectFFmpeg(): Promise<FFmpegInfo> {
        ...
        throw new Error('FFmpeg not found. Please install FFmpeg or specify a custom path.');
    }
    ...
    async validateCommand(command: string): Promise<ValidationResult> {
        ...
        try {
            ...
            const testCommand = command.replace(/(-i\s+)"([^"]+)"/g, '$1"/dev/null"') + ' -f null -';
            const { stderr } = await execAsync(`"${this.ffmpegPath}" ${testCommand}`, { timeout: 5000 });
            ...
            const errors = this.extractErrors(errorObj.stderr || errorObj.message || String(error));
     ...
     */
    async executeCommand(command: string, onProgress?: (progress: number) => void): Promise<ExecutionResult> {
        await this.ensureFFmpegDetected();
    ...
    private parseCodecs(output: string): Codec[] {
        ...
        for (const line of lines) {
            ...
            if (match) {
                ...
                const type = flags[2] === 'V' ? 'video' : flags[2] === 'A' ? 'audio' : 'subtitle';
    ...
    private parseFilters(output: string): Filter[] {
        ...
        for (const line of lines) {
            ...
            const match = line.match(/^\s*([TSC\s]{3})\s+(\S+)\s+(\d+)->(\d+)\s+(.+)$/);
            if (match) {
