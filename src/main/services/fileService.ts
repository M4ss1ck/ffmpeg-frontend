...
class FileService {
    ...
    async validateFile(filePath: string): Promise<FileValidationResult> {
        ...
        try {
            ...
            if (sizeGB > 10) {
                errors.push(
                    `File is too large (${sizeGB.toFixed(1)}GB). Maximum supported size is 10GB.`
                );
            } else if (sizeGB > 2) {
                warnings.push(
                    `Large file detected (${sizeGB.toFixed(1)}GB). Processing may take significant time.`
                );
            }
            ...
            if (errors.length === 0) {
                try {
                    ...
                    if (fileInfo.duration === 0) {
                        warnings.push(
                            'File appears to have zero duration - may be corrupted'
                        );
                    }
                } catch (metadataError) {
    ...
    private parseProbeData(probeData: any): Partial<MediaFileInfo> {
        ...
        // Parse streams
        const streamInfos: StreamInfo[] = streams.map(
